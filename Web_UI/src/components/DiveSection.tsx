...
export const DiveSection: React.FC = () => {
  ...
  // Dissolve animation on scroll: Emerges -> Holds centered -> Dissolves into soft blur
  const opacity = useTransform(scrollYProgress, [0, 0.18, 0.62, 0.88], [0, 1, 1, 0]);
  const scale = useTransform(scrollYProgress, [0, 0.18, 0.62, 0.88], [0.88, 1, 1, 1.12]);
  const filter = useTransform(
    scrollYProgress,
    [0, 0.18, 0.62, 0.88],
    ['blur(18px)', 'blur(0px)', 'blur(0px)', 'blur(28px)']
  );

  return (
    <div ref={containerRef} id="dive" className="relative h-[260vh] bg-black text-white">
